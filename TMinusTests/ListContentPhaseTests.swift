...
enum ListContentPhaseTests {
    @Test("Idle or initial load with no items is loading, with no banner")
    static func initialLoadIsLoading() {
        for phase in [ListPhase.idle, .loading(.initial)] {
    ...
    @Test("Loaded, refresh, or load-more with no items is empty, with no banner")
    static func noItemsWithoutErrorIsEmpty() {
        for phase in [ListPhase.loaded, .loading(.refresh), .loading(.loadMore)] {
            let result = ListContentPhase<Int>.resolve(phase: phase, items: [])
