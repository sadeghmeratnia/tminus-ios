...
extension Array where Element: Identifiable {
    ...
    func merging(_ incoming: [Element]) -> [Element] {
        ...
        for item in incoming where !seenIDs.contains(item.id) {
            // Reads back through `incomingByID` rather than appending `item` directly, so a
            // duplicate ID *within* `incoming` itself resolves to the same "latest wins" entry
            // this function already guarantees for IDs that collide with an existing element.
            merged.append(incomingByID[item.id] ?? item)
